...
def format_output(pomodoro_data):
    if pomodoro_data["state"] != "null":
        return "{is_paused}{remaining} ".format(**format_pomodoro_data(
            pomodoro_data
        ))
    else:
        if int(time.time()) % 2 == 0: 
            return "%{B#f00}%{F#fff}00:00 %{B- F-}"
        return "00:00 "
...
def toggle():
    pomodoro = get_pomodoro_proxy()
    if not pomodoro.State or pomodoro.State == 'null':
        pomodoro.Start()
    elif pomodoro.IsPaused:
        pomodoro.Resume()
    else:
        pomodoro.Pause()
